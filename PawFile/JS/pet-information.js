...
document.getElementById("save-information").addEventListener("click", async (e) => {
    ...
    try {
        ...
        for (const form of allForms) {
            ...
            const petData = {
                Microchip_No: form.getAttribute('data-microchip') || generateMicrochipNo(), // ensure unique microchip ID
        ...
        const payload = {
            pets: pets,
            sponsor: { Sponsor_ID: sessionStorage.getItem('userId') }
        };
...
// Generates a random 15-digit numeric microchip number as a string
function generateMicrochipNo() {
    let microchip = '';
    for (let i = 0; i < 15; i++) {
        microchip += Math.floor(Math.random() * 10);
    }
    return microchip;
}
